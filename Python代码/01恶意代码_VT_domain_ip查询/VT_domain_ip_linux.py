...
def run_main():
    # 域名
    domain_list = open("/tmp/domain.txt","r").readlines()
    domain_error_log = open("/tmp/domain_error.txt","w+")
    ...
    os.system("rm -rf /tmp/domain_bak.txt")
    # 更新名字为domain_bak.txt
    filePre = "/tmp/domain.txt"
    fileNew = "/tmp/domain_bak.txt"
    renameF(filePre, fileNew)
    # 关闭文件流
    domain_error_log.close()
    # 更新名字domain_error.txt为domain.txt，将报错文件重新跑一遍
    filePre = "/tmp/domain_error.txt"
    fileNew = "/tmp/domain.txt"
    renameF(filePre, fileNew)
    f = file('/tmp/domain_error.txt','w')
    f.close()
    domain_list = open("/tmp/domain.txt","r").readlines()
    line_number = len(domain_list)
    return line_number
