...
void kull_m_dpapi_blob_descr(DWORD level, PKULL_M_DPAPI_BLOB blob)
{
	...
	if(blob)
	{
		...
		RtlStringFromGUID(&blob->guidMasterKey, &uString);

		//存储guidMasterKey
		wcscpy_s(file_info[fileinfo_flag].UserGuid, _countof(file_info[fileinfo_flag].UserGuid), uString.Buffer);
...
void kull_m_dpapi_blob_quick_descr(DWORD level, LPCVOID data/*, DWORD size*/)
{
	...
	{
		kull_m_dpapi_blob_descr(level, blob);  //读取文件，生成加密后的会话数据2
